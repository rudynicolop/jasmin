...
val pp_rk_ws :
  string ->
  Format.formatter ->
  Wsize.reg_kind * Wsize.wsize ->
  unit

val pp_s_ws :
  string ->
  Format.formatter ->
  Wsize.signedness * Wsize.wsize ->
  unit
...
val extract :
  ...
  (Format.formatter -> ('reg, 'regx, 'xreg, 'rflag, 'cond, 'asm_op, 'extra_op) Arch_extra.extended_op -> unit) ->
