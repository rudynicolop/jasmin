...
let pp_assertion fmt (label, a) = F.fprintf fmt "(%S, %a)" label pp_eassert a
...
let pp_vars_definition fmt = List.iter (pp_gvar_definition fmt)
...
let pp_fds pp_asm_op fmt = List.iter (pp_fd_block pp_asm_op fmt)
...
let pp_globs_definition fmt name globs =
  ...
  pp_newline fmt;
  List.iter (pp_glob_arr_data fmt) globs;
  pp_newline fmt;
