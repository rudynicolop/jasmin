...
let find_proofs_dir () =
  let rec walk dir =
    ...
    if Sys.file_exists cand && Sys.is_directory cand
       && Sys.file_exists (Filename.concat cand "_CoqProject")
    then cand
    else
      let parent = Filename.dirname dir in
      if parent = dir then begin
        prerr_endline "Could not locate Jasmin proofs/ directory; \
                       set JASMIN_PROOFS_DIR.";
        exit 2
      end else walk parent
  in
  walk (Sys.getcwd ())
...
let rocq_check vfile =
  ...
  let args =
    "rocq" :: "c" ::
    r "lang" @ r "compiler" @ r "arch" @ r "3rdparty" @ r "ssrmisc"
...
let load_and_print n name =
  ...
  match
    ToRocq.extract ~imports:true ~split:false
